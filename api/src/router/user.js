...
router.get('/search/:fname', async (req, res) => {
    ...
    try {
        const searchFieild = req.params.fname;
        User.find({ fname: { $regex: searchFieild, $options: '$i' } })
            .then((data) => {
                return res.status(200).json({
                    data,
                });
...
router.get('/user/search/:fname', async (req, res) => {
    try {
        var response = [];
        const fname = req.params.fname
        if (req.params.fname) {
            response = await User.find({ fname: fname });
            res.send(response)
        }
        return response
    } catch (error) {
...
router.get('/searching', (req, res) => {
    ...
    const searchFieild = req.body.fname;
    //console.log(searchFieild)
    User.find({ fname: { $regex: searchFieild, $options: '$i' } })
        .then(data => {
            res.send(data);
